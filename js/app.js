...
function onReady() {
    ...
    if (window.localStorage) {
        nameField.value = window.localStorage.getItem('defaultName');//mapping the input to defaultName
    }
...
function onSubmit(eventObject) {
    ...
    var name = this.elements['name'].value;//refers to ageForm b/c its raising the event
...
function displayAge(name, age) {
    ...
    displayMessage(name + ', you are ' + age + ' years old!');//is error is coerced to false cause value not passed
} //displayAge()
...
function displayMessage(message, isError) {
    ...
    msgElem.className = isError ? 'alert alert-danger' : 'alert alert-success'; //if isError is true assign
                                                                                //alert to danger, if false assign to success
